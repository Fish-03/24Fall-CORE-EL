...
class Trainer:
    ...
    def eval(self, data=None):
        ...
        with torch.no_grad():
            for batch in validation_data:
                ...
                loss_list, interaction = self.game(*batch)
                optimized_loss, accuracy = loss_list
                if (
    ...
    def train_epoch(self):
        ...
        for batch_id, batch in enumerate(self.train_data):
            ...
            with context:
                loss_list, interaction = self.game(*batch)
                optimized_loss, accuracy_ = loss_list
        ...
        full_interaction = Interaction.from_iterable(interactions)
        return [mean_loss.item(),accuracy], full_interaction

    def train(self, n_epochs):
        for callback in self.callbacks:
            callback.on_train_begin(self)
        pbar = tqdm(range(self.start_epoch, n_epochs), desc="Training")
        typeofgame = type(self.game).__name__
        
        for epoch in pbar:
            for callback in self.callbacks:
                callback.on_epoch_begin(epoch + 1)

            losslist, train_interaction = self.train_epoch()
            train_loss, accuracy = losslist
            # pbar.write(
            #     f"Epoch {epoch + 1}/{n_epochs}: train_loss={train_loss:.4f}"
            # )
            pbar.set_description(
                f"Epoch {epoch + 1}/{n_epochs}: train_loss={train_loss:.10f}, train_accuracy={accuracy:.10f}"
            )
            wandb.log({"game_"+typeofgame + "/train_loss": train_loss})
            wandb.log({"game_"+typeofgame + "/train_accuracy": accuracy})
