...
class WandBLogger:
    def __init__(self, project_name="egg", run_name="try5"):
        # Initialize WandB
        wandb.init(project=project_name, name=run_name)
    
    def log_train_loss(self, epoch, loss, lr):
        # Log train loss at the end of each epoch
        wandb.log({
            "epoch": epoch,
            "train_loss": loss,
            "learning_rate": lr
    ...
    def log_validation_loss(self, epoch, loss):
        # Log validation loss at the end of each validation
        wandb.log({
            "epoch": epoch,
...
class Trainer:
    ...
    def train_epoch(self):
        ...
        self.wandb_logger.log_train_loss(self.start_epoch, mean_loss, self.optimizer.param_groups[0]["lr"])
        return mean_loss.item(), full_interaction
