...
wandb.init(project="egg", name="try20")
...
datapath = 'data/guess_position_20.json'
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return map.view(1, 20, 20), ground_truth.view(1, 20, 20)
...
for data in visualize_data_:
    visualize_data([data['map'], data['ground_truth']], path=f"visualize/visualize20/{data['id']}.png")
...
class Vision(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(50, 100, 3, 1, padding=1)
        self.fc1 = nn.Linear(20 * 20 * 100, 500)  

    def forward(self, x):
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        x = F.relu(self.conv3(x))
        x = x.view(-1, 100 * 20 * 20)  
        x = F.relu(self.fc1(x))
        return x
...
if os.path.exists('pretrain20.pth'):
    class_prediction.load_state_dict(torch.load('pretrain20.pth'))
if os.path.exists('vision20.pth'):
    vision.load_state_dict(torch.load('vision20.pth'))



from tqdm import tqdm

if not os.path.exists('pretrain20.pth'):
    ...
    # 保存模型
    torch.save(class_prediction.state_dict(), 'pretrain20.pth')
    # 保存vision
    torch.save(vision.state_dict(), 'vision20.pth')
...
plt.scatter(outputs_pca[:, 0], outputs_pca[:, 1], alpha=0.20)
plt.title("PCA of the pretrained model 20x20")
plt.savefig("pca_pretrain_20.png")
...
output = output.view(-1, 20, 20)
target = target.view(-1, 20, 20)
for i in range(10):
    visualize_data([data[i].cpu().detach().numpy().squeeze(), target[i].cpu().detach().numpy().squeeze(), output[i].cpu().detach().numpy().squeeze()], path=f"pretrain20/pretrain_{i}.png")
...
class Receiver(nn.Module):
    def __init__(self, input_size):
        ...
        self.fc = nn.Linear(input_size, 400)
...
if os.path.exists('game20.pth'):
    game.load_state_dict(torch.load('game20.pth'))
else:
    game = core.SymbolGameGS(sender, receiver, loss)
    ...
    # 保存模型
    torch.save(game.state_dict(), 'game20.pth')
...
for z in test_loader:
    ...
    output = output.view(-1, 20, 20)
    target = target.view(-1, 20, 20)
    message = message.view(-1, 20, 20)

    if cnt < 10:
        

        visualize_data([data[0].cpu().detach().numpy().squeeze(), target[0].cpu().detach().numpy().squeeze(), output[3].cpu().detach().numpy().squeeze(), message[0].cpu().detach().numpy().squeeze()], path=f"symbol_20/symbol_{cnt}.png")
